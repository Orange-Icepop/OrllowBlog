...
{% note info::在遇到这个问题之前，也出现过同样的症状，不一样的是DVI接口也发病，最后打开了安全启动以同时关掉CSM和打开UEFI解决的问题。可以看这篇文章：[解决英伟达旧显卡开启UEFI后无POST和BIOS输出问题](/2025/02/11/解决英伟达旧显卡开启UEFI后不显示POST与BIOS画面的问题/)。 %}
