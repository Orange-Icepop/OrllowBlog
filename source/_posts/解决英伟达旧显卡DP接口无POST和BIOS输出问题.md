...
{% note info %}
在遇到这个问题之前，也出现过同样的症状，不一样的是DVI接口也发病，最后打开了安全启动以同时关掉CSM和打开UEFI解决的问题。可以看这篇文章：[解决英伟达旧显卡开启UEFI后无POST和BIOS输出问题](/2025/02/11/解决英伟达旧显卡开启UEFI后不显示POST与BIOS画面的问题/)。
{% endnote %}
...
{% note info %}
UEFI GOP（图形输出协议）是UEFI固件的一部分，用于在系统启动时初始化和控制图形硬件。它提供了一种标准化的方法来处理图形输出，使得不同的硬件和操作系统可以更好地兼容和协作。
{% endnote %}
...
然后，把备份出来的VBIOS文件(.rom)拖到GOPupd.bat上，程序会自动解压VBIOS并弹出操作窗口。正常情况下只要键入一个y并回车就行，程序会把新的.rom文件放在程序同目录下，文件名后面会添加`_updGOP`方便辨识。
...
下载好NVFLASH之后，解压，将之前GOPUpd工具制作出来的.rom文件拖到`NVFLASH.exe`的同一目录下。

然后就是在此处打开命令行。Windows11用户可以直接右击NVFLASH当前所在目录的空白区域选择打开终端，其它版本的用户可以像之前那样打开cmd，然后使用`cd [路径]`命令导航到NVFLASH所在目录。
