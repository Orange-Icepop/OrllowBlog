...
想要解决其实相当简单，溜一遍参考文档，找到了一个好东西：match键。通过这个玩意儿，我们可以直接将一份netplan配置绑定到指定MAC地址的设备上（当然也可以通过设备名绑定，但是那样不大符合我们的初衷）。

例如，我的netplan配置文件（位于/etc/netplan下面的某个yaml文件）是这样的：
...
其中，match键就是用来匹配设备的。只要把你的MAC地址填到macaddress里，冷启动时系统就会认得这个设备，然后就不会再自己创建新的配置文件了。
