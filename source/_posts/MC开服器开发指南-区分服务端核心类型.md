...
比较需要注意的是，某些核心在不同的版本中会具有不同的主类名称，例如原版核心的最近几个版本（实测最晚为1.21）的主类名称从`net.minecraft.server.MinecraftServer`换为了`net.minecraft.bundler.Main`，但是客户端的主类没改。
...
为了解决这个问题，我在0.08.1版本中引入了`SharpZipLib`库，这个库不会理会时间戳异常，因此完美规避掉了这个问题。完整实现代码如下：
